...
public class Homework1 {
    ...
    public void streamPipeline3() {
        countries.stream()
                .map(Country::getName)
                .filter(name -> name.toLowerCase().charAt(0) == name.toLowerCase().charAt(name.length() -1))
...
ed by the language code "es").
    ...
    public void streamPipeline15() {
        countries.stream()
                .filter(country -> country.getArea() != null &&  country.getArea().compareTo(BigDecimal.ONE) == -1)
                .map(Country::getName)
                .forEach(System.out::println);
    }
